...
构建过程只读取配置 subjects 列表中的笔记、题图和已生成的 Question ID 索引，不修改、移动或删除原笔记和原图。所有媒体必须通过仓库内安全路径检查，输出包括：
...
`.apkg` 默认被 `.gitignore` 排除；Anki 负责主要间隔重复调度，仓库 Question ID 负责内容身份、错因与诊断，二者不维护互相独立的主调度器。
